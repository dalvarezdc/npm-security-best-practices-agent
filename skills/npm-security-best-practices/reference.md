...
### 1.4 pnpm trust policy (pnpm 10.21+)
...
# Ignore the check for packages published >30 days ago (pnpm 10.27+)
...
### 2.1 pnpm blockExoticSubdeps (pnpm 10.26+)
...
**Hardened pattern — pre-install then run offline:**
```bash
...
# Step 2: Run with offline mode — refuses any live registry fetch
...
```bash
cd $HOME/mcp && npm update
# Review lockfile changes before next npx invocation
```
...
## 13. Publish with OIDC (Trusted Publisher)
...
## 16. Do Not Trust the Official npmjs.org Registry UI
